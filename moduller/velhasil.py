...
class Velhasil ():
    ...
    def __init__(self, text=""):
        misspelled_words_file = "data/turkish_misspellings1.txt"

        # Ensure the file is UTF-8 encoded
        with open(misspelled_words_file, 'r', encoding='utf-8') as file:
            content = file.read()
        with open(misspelled_words_file, 'w', encoding='utf-8') as file:
            file.write(content)

        self.fsm = FsmMorphologicalAnalyzer("data/turkish_dictionary.txt",
    ...
    def cumleBulucu(self, paragraflar):
        ...
        for sayac, paraf in enumerate(paragraflar):
            if paraf != "":
                ...
                sentence = re.compile(r"""(?<=['""a-zıüöşğç""\]\)][\!\?\:\.\…\n\r\n\t])\s+(?=[""A-ZİÜÖŞĞÇ0-9\(\-\(\''\'\""\""\[\+])""")
